...
function cast() {
    ...
    Livewire.dispatch('player:cast', {impulse, sphereIdx});
...
function playerCollisions() {
    ...
    if (result) {
        ...
        Livewire.dispatch('player:collision', value);
    }
...
function handleKeyStates(deltaTime) {
    ...
    if (keyStates['KeyW']) {
        ...
        Livewire.dispatch('player:forward', keyStates['KeyW']);
        moving = true;
    }

    if (keyStates['KeyS']) {
        playerVelocity.add(getForwardVector().multiplyScalar(-speedDelta));
        Livewire.dispatch('player:back', keyStates['KeyS']);
        moving = true;
    }

    if (keyStates['KeyA']) {
        playerVelocity.add(getSideVector().multiplyScalar(-speedDelta));
        Livewire.dispatch('player:left', keyStates['KeyA']);
        moving = true;
    }

    if (keyStates['KeyD']) {
        playerVelocity.add(getSideVector().multiplyScalar(speedDelta));
        Livewire.dispatch('player:right', keyStates['KeyD']);
        moving = true;
    }
    ...
    if (playerOnFloor) {
        if (keyStates['Space']) {
            ...
            Livewire.dispatch('player:jump', playerVelocity.y);
        }
    }
}
